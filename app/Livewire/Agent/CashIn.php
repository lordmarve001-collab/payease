...
class CashIn extends Component
{
    ...
    public function continueToPin(): void
    {
        ...
        if (!$this->canProceed) {
            $this->dispatch('notify-error', message: $this->validationMessage !== '' ? $this->validationMessage : 'Please fix the deposit details and try again.');
            return;
        }
    ...
    public function confirmDeposit(): void
    {
        ...
        if (!$this->canProceed) {
            $this->dispatch('notify-error', message: $this->validationMessage !== '' ? $this->validationMessage : 'Unable to continue with this deposit.');
            return;
        }

        if (!$this->verifyAgentPin('cash_in')) {
            return;
        }
        ...
        try {
            ...
            if (!$agent || !$customer) {
                throw new RuntimeException('Unable to prepare this cash-in transaction.');
            }

            /** @var TransactionService $transactionService */
            $transactionService = app(TransactionService::class);
            $transaction = $transactionService->processCashIn($agent, $customer, (float) $this->amount);
            ...
            $this->newCustomerBalance = (float) ($transaction->toWallet?->available_balance ?? 0);
            $this->newFloatBalance = (float) $agentUser->fresh()->agent?->float_balance;
            $this->resultState = 'success';
            $this->resultMessage = '';
            $this->step = 4;
            $this->dispatch('notify-success', message: 'Cash in completed successfully.');
    ...
    {
        return view('livewire.agent.cash-in')->layout('components.layouts.agent');
    }
    ...
    protected function syncPreview(): void
    {
        ...
        /** @var TransactionService $transactionService */
        $transactionService = app(TransactionService::class);
        $preview = $transactionService->getAgentCashInPreview($agent, $customer, (float) $this->amount);
