...
class SendFund extends Component
{
    ...
    {
        $lockKey = 'ajo_owner_pin_lock_send_' . auth()->id();
    ...
    protected function loadBanks(): void
    {
        ...
        try {
            ...
            Log::channel('monnify')->warning('Failed to fetch banks from Monnify, using fallback', [
                'error' => $e->getMessage(),
            ]);
            $this->banks = $fallback;
        }
    ...
    protected function lookupAccountName(): void
    {
        ...
        try {
            ...
            $this->accountName = '';
            $this->accountNameError = $e->getMessage() !== 'Could not resolve account name.'
                ? 'Could not verify account name. Please check the details.'
                : $e->getMessage();
            Log::channel('monnify')->warning('Account name lookup failed', [
    ...
    public function continueToConfirm(): void
    {
        ...
        if (!$this->canProceed) {
            $this->dispatch('notify-error', message: $this->validationMessage !== '' ? $this->validationMessage : 'Please fix the details.');
            return;
        }

        if ($this->recipientType === 'bank') {
            if (strlen($this->accountNumber) !== 10) {
                $this->dispatch('notify-error', message: 'Enter a valid 10-digit account number.');
                return;
            }
            if (!$this->selectedBankCode) {
                $this->dispatch('notify-error', message: 'Select a bank.');
                return;
            }
            if (!$this->accountName) {
                $this->dispatch('notify-error', message: 'Verify the account name first.');
                return;
            }
        }
    ...
    public function continueToPinStep(): void
    {
        ...
        if (!$this->canProceed) {
            $this->dispatch('notify-error', message: $this->validationMessage !== '' ? $this->validationMessage : 'Cannot proceed.');
            return;
        }
    ...
    public function confirmTransferPin(): void
    {
        ...
        if ($this->getPinLockSeconds() > 0) {
            $this->pinError = 'Send is locked for ' . $this->getPinLockSeconds() . ' seconds.';
            return;
        }
        ...
        $attemptKey = 'ajo_owner_pin_attempts_send_' . $user->id;
        $lockKey = 'ajo_owner_pin_lock_send_' . $user->id;

        if (Cache::get($lockKey) && Cache::get($lockKey) > now()->timestamp) {
            $this->pinError = 'Send is locked. Try again later.';
            $this->dispatch('focus-transfer-pin');
            return;
        }
    ...
    public function render()
    {
        ...
        return view('livewire.ajo-owner.send-fund', [
            'wallet' => $wallet,
    ...
    protected function shouldDisplayMessage(string $message): bool
    {
        ...
        if ($message === 'Enter a recipient phone number.') return false;
        if ($message === 'Enter an amount to continue.' && $this->amount === '') return false;
        if ($message === 'Recipient not found.' && strlen($this->phone) < 10) return false;
        return true;
    }
