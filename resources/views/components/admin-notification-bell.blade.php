<div class="relative" x-data="{ open: false }">
    <button @click="open = !open" class="relative p-2 text-text-secondary hover:text-text-primary transition-colors rounded-full hover:bg-surface-2 cursor-pointer">
    ...
    <div x-show="open" @click.outside="open = false" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="absolute right-0 top-full mt-2 w-80 sm:w-96 glass-strong rounded-card shadow-elevation-4 border border-white/10 z-50 overflow-hidden" x-cloak>
        <div class="p-3 border-b border-border flex items-center justify-between bg-surface-2/40">
        ...
        <div class="max-h-80 overflow-y-auto divide-y divide-border">
            @forelse($notifications as $notification)
                <div class="px-3 py-3 hover:bg-surface-2/50 transition-colors {{ $notification->is_read ? 'opacity-60' : '' }}">
        ...
        <div class="p-2 border-t border-border text-center bg-surface-2/40">
