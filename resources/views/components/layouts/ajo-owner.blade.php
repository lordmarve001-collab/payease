...
<body class="bg-background text-text-primary font-sans antialiased min-h-screen pb-20 md:pb-0">
    ...
    <header class="fixed top-0 left-0 right-0 h-16 bg-surface border-b border-border z-30 flex items-center justify-between px-4 md:ml-[72px] lg:ml-[240px] transition-all duration-300">
        <div class="flex items-center gap-2 md:hidden">
            ...
            @else
                <div class="w-8 h-8 rounded-full bg-purple-600 flex items-center justify-center text-white shrink-0">
                    <x-lucide-users class="w-4 h-4" />
                </div>
                <span class="font-bold text-lg text-purple-600">{{ $siteSettings->site_name ?? 'PayEase' }}</span>
            @endif
            <span class="bg-purple-100 text-purple-700 text-[10px] font-bold px-1.5 py-0.5 rounded tracking-wide uppercase">{{ __('Ajo Owner') }}</span>
        </div>
        <div class="hidden md:flex items-center gap-2">
            <span class="bg-purple-100 text-purple-700 text-[10px] font-bold px-1.5 py-0.5 rounded tracking-wide uppercase">{{ __('Ajo Owner Portal') }}</span>
        </div>
        ...
        <div class="flex items-center gap-4 ml-auto">
            ...
            <a href="{{ route('ajo-owner.profile') }}" wire:navigate class="w-8 h-8 rounded-full bg-purple-600 text-white flex items-center justify-center font-bold text-sm shadow-sm cursor-pointer">
    ...
    <x-sidebar>
        ...
        <div class="mt-auto pt-2 border-t border-border">
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-btn text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-600 transition-colors text-sm font-medium">
    ...
    <x-bottom-nav>
        ...
        <a href="{{ route('ajo-owner.groups.create') }}" wire:navigate class="relative -top-5 flex flex-col items-center justify-center gap-1 text-text-secondary hover:text-purple-600 transition-colors">
            <div class="w-14 h-14 rounded-full bg-purple-600 text-white flex items-center justify-center shadow-elevation-2 active:scale-95 transition-transform border-4 border-surface {{ request()->is('ajo-owner/groups/create') ? 'ring-2 ring-purple-600 ring-offset-2 ring-offset-background' : '' }}">
