...
<body class="app-bg text-text-primary font-sans antialiased min-h-screen pb-20 md:pb-0">
    ...
    <header class="fixed top-0 left-0 right-0 h-16 bg-surface/70 backdrop-blur-xl border-b border-border z-30 flex items-center justify-between px-4 md:ml-[72px] lg:ml-[240px] transition-all duration-300">
        <div class="flex items-center gap-2 md:hidden">
            ...
            @endif
            <span class="bg-secondary/20 text-accent text-[10px] font-bold px-1.5 py-0.5 rounded tracking-wide uppercase">Agent</span>
        </div>
        <div class="hidden md:flex items-center gap-2">
            <span class="bg-secondary/20 text-accent text-[10px] font-bold px-1.5 py-0.5 rounded tracking-wide uppercase">Agent Portal</span>
        </div>
        ...
        <div class="flex items-center gap-4 ml-auto">
            ...
            <div class="w-8 h-8 rounded-full bg-gradient-to-br from-secondary to-accent text-white flex items-center justify-center font-bold text-sm shadow-glow-secondary">
    ...
    <x-bottom-nav>
        ...
        <a href="{{ route('agent.cash-out') }}" wire:navigate class="relative -top-5 flex flex-col items-center justify-center gap-1 text-text-secondary hover:text-accent transition-colors">
            <div class="w-14 h-14 rounded-full bg-gradient-to-br from-secondary to-accent text-white flex items-center justify-center shadow-glow-secondary active:scale-95 transition-transform border-4 border-surface {{ request()->is('agent/cash-out') ? 'ring-2 ring-secondary ring-offset-2 ring-offset-background' : '' }}">
