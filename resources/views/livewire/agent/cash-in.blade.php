<div class="px-4 py-6 md:p-8 max-w-lg mx-auto relative overflow-hidden">
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-text-primary">{{ __('Cash In (Deposit)') }}</h1>
            <p class="text-text-secondary text-sm">{{ __('Deposit funds into a customer\'s account.') }}</p>
    </div>
    ...
    <div x-data x-transition:enter="transition ease-material duration-300 transform"
        ...
        <div class="pt-4">
            <x-button variant="primary" size="large" wire:click="lookupCustomer" wire:loading.attr="disabled" x-bind:disabled="@js(strlen($phone) < 10)" class="w-full relative">
    ...
    <div x-data x-transition:enter="transition ease-material duration-300 transform"
        ...
        <div class="bg-surface rounded-card p-4 shadow-sm border border-border flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 bg-primary-light text-primary rounded-full flex items-center justify-center">
        ...
        <div class="bg-gray-50 rounded-card p-3 border border-border flex items-center justify-between text-sm">
                    <span class="text-text-secondary">{{ __('Current Balance') }}</span>
            <span class="font-semibold text-text-primary">₦{{ number_format($customerBalance, 2) }}</span>
        </div>

        <div class="pt-2 text-center">
            <label class="block text-sm font-medium text-text-secondary mb-2">{{ __('Deposit Amount') }}</label>
        ...
        <div class="flex justify-center gap-2 flex-wrap">
            <button wire:click="setAmount(1000)" class="px-4 py-2 rounded-full border border-border text-sm font-medium hover:bg-primary-light hover:border-primary hover:text-primary transition-colors">₦1K</button>
            <button wire:click="setAmount(5000)" class="px-4 py-2 rounded-full border border-border text-sm font-medium hover:bg-primary-light hover:border-primary hover:text-primary transition-colors">₦5K</button>
            <button wire:click="setAmount(10000)" class="px-4 py-2 rounded-full border border-border text-sm font-medium hover:bg-primary-light hover:border-primary hover:text-primary transition-colors">₦10K</button>
            <button wire:click="setAmount(20000)" class="px-4 py-2 rounded-full border border-border text-sm font-medium hover:bg-primary-light hover:border-primary hover:text-primary transition-colors">₦20K</button>
        </div>
        ...
        @if($amount > 0)
        <div class="bg-primary-light border border-primary/20 rounded-card p-4 text-center mt-4 animate-[fade-in_0.3s_ease-out]">
            <p class="text-primary-dark text-sm font-medium mb-1">{{ __('Your Commission') }}</p>
            <p class="text-2xl font-bold text-primary">₦{{ number_format($commission, 2) }}</p>
        </div>
        @endif

        <div class="space-y-3 pt-4">
            <x-button variant="primary" size="large" wire:click="continueToPin" wire:loading.attr="disabled" x-bind:disabled="@js(!$canProceed)" class="w-full">
                {{ __('Continue') }}
            </x-button>
    ...
    <div x-data x-transition:enter="transition ease-material duration-300 transform"
        ...
        <div class="bg-surface rounded-card p-4 shadow-sm border border-border space-y-3">
            ...
            <div class="flex justify-between text-sm">
                <span class="text-text-secondary">Deposit Amount</span>
                <span class="font-bold text-text-primary">₦{{ number_format((float) $amount, 2) }}</span>
            </div>
            <div class="flex justify-between text-sm">
                <span class="text-text-secondary">Commission</span>
                <span class="font-bold text-primary">₦{{ number_format($commission, 2) }}</span>
            </div>
        </div>

        <div>
            <label class="block text-sm font-medium text-text-primary mb-2">{{ __('Enter Agent PIN') }}</label>
            <input type="password" wire:model.live="agentPin" inputmode="numeric" maxlength="6" class="block w-full rounded-btn border border-border px-4 py-3 outline-none focus:border-primary focus:ring-primary" placeholder="••••••">
        ...
        <div class="space-y-3 pt-4 relative">
            ...
            <x-button variant="primary" size="large" wire:click="confirmDeposit" wire:loading.attr="disabled" x-bind:disabled="@js(strlen($agentPin) !== 6)" class="w-full">
                {{ __('Confirm Deposit') }}
            </x-button>
            <x-button variant="secondary" size="large" wire:click="goBack" wire:loading.attr="disabled" class="w-full bg-gray-100 text-text-primary hover:bg-gray-200">
                {{ __('Back') }}
    ...
    <div x-data x-transition:enter="transition ease-material duration-300 transform delay-300"
        ...
        <div>
            <h2 class="text-2xl font-bold text-text-primary">{{ $resultState === 'failed' ? __('Deposit Failed') : __('Deposit Successful!') }}</h2>
            @if($resultState === 'failed')
                <p class="text-sm text-text-secondary mt-1">{{ $resultMessage }}</p>
            @endif
        </div>
        ...
        <div class="bg-surface shadow-sm border border-border rounded-card p-4 space-y-3 text-sm text-left">
            <div class="flex justify-between border-b border-border pb-3">
                <span class="text-text-secondary">{{ __('Amount Deposited') }}</span>
                <span class="font-bold text-text-primary">₦{{ number_format((float) $amount, 2) }}</span>
            </div>
        ...
        <div class="{{ $resultState === 'failed' ? 'bg-red-50 border-red-200' : 'bg-primary-light border-primary/20' }} border rounded-card p-3 flex justify-between items-center">
            <span class="{{ $resultState === 'failed' ? 'text-danger' : 'text-primary-dark' }} font-medium">
                {{ $resultState === 'failed' ? __('Transaction Status') : __('Commission Earned') }}
            </span>
            <span class="text-lg font-bold {{ $resultState === 'failed' ? 'text-danger' : 'text-primary' }}">
        ...
        <div class="space-y-3 pt-4">
            @if($resultState === 'failed')
                <x-button variant="primary" size="large" wire:click="tryAgain" class="w-full">
                    {{ __('Try Again') }}
                </x-button>
            @else
